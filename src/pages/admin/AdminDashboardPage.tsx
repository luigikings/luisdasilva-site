...
import { getAdminQuestions, getMetrics } from '../../services/adminApi'
    ...
    <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-8 px-4 py-10">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg">
          ...
          <p className="mt-3 text-3xl font-semibold text-slate-100">
            {metrics ? numberFormatter.format(metrics.activeQuestions) : '—'}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg">
      ...
      <section className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">Total: {numberFormatter.format(questions.length)}</span>
          </div>
                ...
                {isLoading ? (
                  <tr>
                    <td className="px-3 py-6" colSpan={4}>
                      <p className="text-center text-sm text-slate-500">Cargando preguntas…</p>
                    </td>
                  </tr>
                ) : questions.length === 0 ? (
                  <tr>
        ...
        <div className="space-y-4">
              ...
              {metrics && metrics.topQuestions.length > 0 ? (
                metrics.topQuestions.map((question) => (
                  <li key={question.id} className="flex items-start justify-between gap-4">
                    <span className="flex-1 leading-tight">{question.text}</span>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg">
            ...
            {metrics && Object.keys(metrics.clicksByCategory).length > 0 ? (
              <ul className="space-y-2 text-sm text-slate-300">
                {Object.entries(metrics.clicksByCategory).map(([category, total]) => (
