...
export async function sendDoorEntryEmail(payload: DoorEntryEmailPayload = {}): Promise<DoorEntryEmailResult> {
  const transportConfig = getTransportConfig();
  ...
  try {
    ...
    console.error('Failed to send door entry email.', error);
    throw error;
  }
